...
comments. Any line beginning with the value of `commentCharacter` attribute (`+++#+++` by
default) will be treated as a comment and ignored. Note, however, that the comment
character must be the first character on the line without any leading whitespace.
It is therefore recommended that the closing text block delimiter (`"""`) be placed
either at the end of the last line of input or on the following line,
...
NOTE: Any line beginning with the value of `commentCharacter` attribute (`+++#+++` by
default) will be interpreted as a comment and will be ignored.
