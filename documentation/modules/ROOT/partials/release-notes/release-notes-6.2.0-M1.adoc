...
* `ConsoleLauncher` with `--details=verbose` now reports the duration of each test and
  indents its output correctly when tests are executed in parallel. Previously, the
  reported durations belonged to other executions and output lines could interleave.
