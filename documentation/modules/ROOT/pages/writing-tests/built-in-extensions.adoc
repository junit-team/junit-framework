...
The system `Properties` object normally acts like a map of strings. While strongly discouraged, it is possible to use non-string keys and values. It is also possible to create {jdk-javadoc-base-url}/java.base/java/util/Properties.html#%3Cinit%3E(java.util.Properties)[nested `Properties` with inherited default values]. `@RestoreSystemProperties` restores the original `Properties` object with all of its potential richness _after_ the annotated scope is complete.
However _during_ the execution of the annotated scope, the system properties are set to a cloned `Properties` object with these limitations:

- Properties with non-string values are removed.
- Nested `Properties` are flattened into a non-nested instance that has the same effective values.
