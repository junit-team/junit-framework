...
The System Properties extension consists is a set of annotations that work together to read, write and clear system properties.  
...
the execution of the annotated scope, the system properties are set to a cloned `Properties` object.
However it is possible to create a {jdk-javadoc-base-url}/java.base/java/util/Properties.html#%3Cinit%3E(java.util.Properties)[nested `Properties` with inherited default values] that are not included in the cloned object. The extension will perform a best effor attempt to  detect and fail if default values are detected. For classes that extend `Properties` it is assumed that `clone()` is implemented with sufficient fidelity.
...
TODO: Link first use ot Java doc.
