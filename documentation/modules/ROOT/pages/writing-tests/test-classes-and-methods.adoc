...
[[test-classes-and-methods-kotlin-coroutines]]
== Kotlin Coroutines
...
Traditionally, one would use `runBlocking` to bridge the gap between tests and coroutines. However,
`runBlocking` does not support skipping calls to `delay`. In addition, it does not support testing
code that requires control over the execution time or the dispatcher. For such cases, using
standard test methods and wrapping the test body in `runTest` from
