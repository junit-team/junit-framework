...
To assist IDEs with code completion test engines may generate machine-readable overview
of configuration parameters by annotating their configuration parameters with
`{PlatformConfigurationParameter}`.
...
Processing the annotations with `junit-platform-configuration-processor` will produce
a `junit-platform-configuration-metadata.json` in the format of
{SpringBootConfigurationMetadata}.
